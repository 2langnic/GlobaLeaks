GLClient.controller('SubmissionCtrl',
        ['$scope', '$rootScope', '$location', 'Authentication', 'Node', 'Submission', 'Receivers', 'WhistleblowerTip',
            function ($scope, $rootScope, $location, Authentication, Node, Submission, Receivers, WhistleblowerTip) {
                ...
                $scope.all_receivers = Receivers.query();
                ...
                $scope.filterForcefullyReceivers = function (receiver) {
                    if (receiver.configuration === 'globalForcefully') {
                        return true;
                    } else {
                        return false;
                    }
                ...
                $scope.checkForcefully = function () {
                    $scope.checkforcefullyresult = false;
                    var forEach = angular.forEach;
                    forEach($scope.all_receivers, function (receiver) {
                        if (receiver.configuration === 'globalForcefully') {
                             $scope.checkforcefullyresult = true;
                            return;
                        }

                    }
                    );
                    return  $scope.checkforcefullyresult;
                }
