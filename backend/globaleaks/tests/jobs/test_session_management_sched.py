...
class TestSessionManagementSched(helpers.TestGL):
    ...
    def test_session_management_sched(self):
        ...
        authentication.GLSession('admin', 'admin', 'enabled', 'XSRFTOKEN') # 1!
        authentication.GLSession('admin', 'admin', 'enabled', 'XSRFTOKEN') # 2!
        authentication.GLSession('admin', 'admin', 'enabled','XSRFTOKEN') # 3!

        self.assertEqual(len(GLSetting.sessions), 3)
        authentication.reactor.advance(GLSetting.defaults.lifetimes['admin'])  # @UndefinedVariable
